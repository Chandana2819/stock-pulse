  ...
  return (
    <section className="border border-border-bright bg-bg-1 overflow-hidden">
      <div className="py-[0.6rem] px-6 border-b border-border-custom bg-bg-2">
        <span className="font-mono text-[0.62rem] tracking-[0.18em] text-text-3 uppercase">// {greeting()} — YOUR MARKET BRIEF</span>
      </div>
      <div className="p-6 flex flex-col gap-4">
        <div className="flex flex-wrap items-center gap-6">
          <div>
            <div className="font-mono text-[0.55rem] text-text-3 uppercase tracking-widest">Market Risk</div>
          ...
          {brief.portfolio && (
            <div>
              <div className="font-mono text-[0.55rem] text-text-3 uppercase tracking-widest">Your Portfolio</div>
        ...
        {brief.drivers.length > 0 && (
          <div>
            <div className="font-mono text-[0.55rem] text-text-3 uppercase tracking-widest mb-1.5">Moving today</div>
            <div className="flex flex-wrap gap-2">
              {brief.drivers.map((d, i) => (
                <span key={i} className="font-mono text-[0.65rem] px-2.5 py-1 border border-border-custom bg-bg-2 text-text-2">{d}</span>
              ))}
            </div>
          </div>
        )}

        <div className="p-3 border border-cyan-custom/30 bg-blue-dim font-mono text-xs text-text-custom">
          {brief.action}
        </div>
      </div>
    </section>
